...
// adj list String representation
	public abstract String adjacencyString();

	

	
	// create a new map with if code have  label vertex
	public void initializeLabels() {
		vertexLabels = new HashMap<Integer,String>();
	}	
	// checking if a vertex is label or not 
	public boolean hasVertex(int v)
	{
		return v < getNumVertices();
	}
	
// check if some vertex is label or not 
	public boolean hasVertex(String s)
	{
		return vertexLabels.containsValue(s);
	}
	
// add label to unlabeled vertex
...
// in case of error report the current vertex label  //
	...
	public static void main (String[] args) {
		...
		// a grapghLoader class that upload the grap from package called the data and as 
		// well as the intersection data of the maps
		GraphLoader.createIntersectionsFile("data/maps/ucsd.map", "data/intersections/ucsd.intersections");
		

	// to test the degreeSeqeuence method ...//
		System.out.println("Loading graphs based on real base  data...");
		...
		// implemeted by @author ucsd dev team ....//
		...
		// test case to test distance2 emthod codew 
		System.out.println("Testing distance-2 methods on custom  graphs...");
		System.out.println("Goal: implemeted using the adj matrix and list.");
